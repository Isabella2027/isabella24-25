...
static int on_black(int port) {
    return analog(port)>irs.midpoint;
}
...
void demo_gyro_square_up(int speed) {
    ...
    while(!on_black(irs.left) || !on_black(irs.right)) {
        mav(motors.left, on_black(irs.left) ? -0.5*speed : speed);
        mav(motors.right,on_black(irs.right)? -0.5*speed : speed);
        msleep(10);
    }
    freeze(motors.left);
    freeze(motors.right);
